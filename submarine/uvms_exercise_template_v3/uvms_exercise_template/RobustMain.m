...
%task_set = {task_zero_altitude, task_vehicle};
%task_set_landing = {task_altitude, task_alignment, task_vehicle};
task_set_landing = {task_altitude, task_alignment, task_vehicle};

% Define actions and add to ActionManager
actionManager = ActionManager();
%actionManager.addAction(task_set);  % action 1
actionManager.addAction(task_set_landing);  % action 1
...
logger = SimulationLogger(ceil(endTime/dt)+1, robotModel, task_set_landing);
